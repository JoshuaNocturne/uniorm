...
runner 的那只容器为了编驱动早就装好了 ODBC 开发头）；四条腿这副还欠 runner 那一趟，
但内容不是没量过：一只 `ubuntu:24.04` 容器里装着 CI 钉的那两支连接器，对 `8.4.11`
与 `11.8.9` 两台服务端各连一次，四格跑满，五条测试条条全绿。每条腿在构建之前还要问
一句 DSN 背后答的是哪台服务端，与自家名字不符即红。后续为 libpq / Oracle OCI 原生
