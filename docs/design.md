...
- **CI**（已写入 `.github/workflows/ci.yml`；GitHub 上跑绿过的那副形状是三支作业——
  ...
  **服务端答的元数据**：本地拿真 MySQL 服务端跑这套测试，第一趟就撞出 MariaDB 连接器
  ...
  把当前这棵树按数据库那条腿的形状配置（`UNIORM_BACKEND_ODBC=ON`、`UNIORM_BUILD_TOOLS=ON`、
  `-Wall -Wextra`）跑 `ctest -LE perf`，两台服务端各连一次——四格五条全绿，一条 Skip 也没有。
  于是这副新形状欠 runner 的只剩它自己：矩阵选镜像、`health_cmd` 按镜像给、`services`
  里两套 env 前缀，而配错了的配对有一条自己的 banner 预检先红。
...
- ~~打包~~ **已完成（§3.1）**：`install(TARGETS/EXPORT)` + config/version 文件 +
  ...
  **已完成**：`.github/workflows/ci.yml`（§8）的三条形状都已按作业原样在它所要用的镜像里跑过，这一趟
  ...
  四条腿这副还没被 runner 看过——要它确认的是这副形状本身（矩阵选镜像、按镜像给的健康门、
  `services` 里两套 env 前缀；配错了会撞上那条腿自己的 banner 预检，见 §8）。四条腿的
  内容不用等 runner：已在装着 CI 那两支连接器的 `ubuntu:24.04` 容器里按当前构建量过，
  两台服务端各四格五条全绿（§8）。
