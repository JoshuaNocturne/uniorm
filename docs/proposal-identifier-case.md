...
- **C 的策略 + B 的诊断**要一次真库验证：任一腿建一张只与声明差大小写的表（例如活库
  ...
  声明 `lower` 后通过。这条不进 golden 比对，只断言成败。**按更小的形状落了**：不建
  新表、不动夹具，只在已有那条腿的构建器用例末尾拿 `build_select()` 的文本重发一次
  ——`upper` 下表名跟着升上去、原来那条小写断言反过来不成立，验的是"出口只有一处"，
  不执行任何语句。至于"声明错了会响"那一半，本机没有可连的库，两条断言都还没真跑过，
  第一次要在 CI 的腿上。
...
- 三条同族腿在 B 与 C 下都是恒等（夹具全小写、`keep` 默认），golden 不动就是"不改变
  现有语义"的证据。
