...
的那个名字"，只写 `validate()` 按同一策略问目录、落空时报出目录侧的拼法。这句收敛最
初漏了三处，如今一并追平：design.md §4.7 的"校验通过与查询发得出是同一件事"、§4.8
的"校验收的正是查询会发出去的那个名字"，以及 `dialect.hpp` 里那句同义的英文。
...
- **C 的策略 + B 的诊断**要一次真库验证：任一腿建一张只与声明差大小写的表（例如活库
  ...
  列名折法是两桩事实，而一份策略要同时救整份映射，正是本提案的前提，
  `lower_case_table_names=1` 那种表折小写而列名照建表保留的配法就会当场把它
  撞开，而不是让假红炸进 `validate()`。每格的实际拼法无条件记一行 `note:`，为的是
  让 psqlODBC 折不折 pattern、那台服务端又是哪值，第一次跑就看得见——这两格此前
  都没量过。第一次跑（三条腿全绿）量到了断言，却没量到记录：ctest 对通过的用例根本
  不出它的输出，作业因此改成 `-V` 跑。这条不进 golden 比对，只断言成败；`gen_e2e`
  另用自己的夹具，多出的这张表动不到它。
...
拼法"，不写"校验收的就是查询会发出去的那个名字"（§4.3 末段那一格，design.md 的两处
草图起初漏收，见那里）；§6.4 另写明那项检查要把整份目录列一遍，所以配置里没有
`[tables.*]` 段时就不列。
